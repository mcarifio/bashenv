...
); declare -fx os-release

os-release.version_id() (
    os-release $(name2env ${FUNCNAME##*.})
); declare -fx os-release.version_id

os-release.id() (
    os-release $(name2env ${FUNCNAME##*.})
); declare -fx os-release.id

os-release.major() {
    local _major=$(os-release.version_id)
    echo -n ${_major%.*}
}; declare -fx os-release.major

os-release.name-version() ( printf "%s-%s" $(os-release.id) $(os-release.version_id); ); declare -fx os-release.name-version
