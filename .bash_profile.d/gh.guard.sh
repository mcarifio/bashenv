...
gh.repo.create() {
    ...
    local _repo=${1:?'expecting a repo name'}; shift
    local _folder=$2
    local _git_dir=$(2>&1 > /dev/null gh repo create ${_repo} --add-readme --clone --description "tbs ${_repo}" --public "$@" | awk $'match($0, /^Cloning into \'([^\']+)\'\.\.\./, m) { print m[1]; }')
    [[ -n "${_folder}" && -d "${_git_dir}" ]] || mv -v ${_git_dir} ${_folder}
