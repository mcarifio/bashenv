...
main() (
    ...
    local _target=${3:-/etc/fstab} ## the output. set this to something like /tmp/fstab+${_label} to inspect the result first
    ...
    btrfs filesystem show ${_label} || u.warn "${FUNCNAME}@${LINENO} no btrfs filesystem labeled '${_label}', continuing..."
    ...
    if findmnt --fstab --timeout 3000 --mountpoint "${_mountpoint}"; then
        ...
        >&2 echo "${FUNCNAME}@${LINENO} /etc/fstab already has a mountpoint ${_mountpoint}."
        >&2 echo "${FUNCNAME}@${LINENO} You must merge ${_target} and /etc/fstab manually"
    fi
...
EOF
    ...
    (set -x; LIBMOUNT_FSTAB="${_working}" sudo findmnt --verbose --verify) || return $(u.error "${FUNCNAME}@${LINENO}: ${_working} did not verify")
    sudo cp --update --verbose --backup=numbered "${_working}" "${_target}" || true
    >&2 echo "Augmented fstab: ${_target}"
    if [[ /etc/fstab = "${_target}" ]]; then
        [[ -d "${_mountpoint}" ]] && >&2 "mountpoint '${_mountpoint}' exists, continuing..." || sudo install --directory --owner=${USER} --group=${USER} "${_mountpoint}"
...
# for testing:
main store "/store" /tmp/fstab+store
