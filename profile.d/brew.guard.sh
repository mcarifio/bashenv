...
_brew.parse() (
    ...
    set -uEeo pipefail
    shopt -s nullglob
    declare -A _brew_options=([file]="${PWD}/${FUNCNAME}" [comment]="${HOSTNAME}:and_some_stuff" [trace]=0)
    local -a _rest=( $(u.parse _brew_options --foo=bar --user=${USER} --trace 1 2 3) )
    printf '%s ' ${FUNCNAME}; declare -p _brew_options; printf '%s ' ${_rest[@]}
)
